...
def sls_convolution(number_of_disjunction_term_in_SLS_DCDL, maximum_steps_in_SLS_DCDL, stride_of_convolution,
    ...
    if DCDL_train:
        ...
        for channel in range(label.shape[3]):
            ...
            accuracy = (data_flat.shape[0] - found_formula.total_error_on_validation_set) / data_flat.shape[0]
            ...
            if result is not None:
                ...
                label_self_calculated = helper_methods.calculate_convolution(data_flat, used_kernel[:, :, :, channel],
                                                                             result)
